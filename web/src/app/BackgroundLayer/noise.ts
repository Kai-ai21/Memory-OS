/**
 * Perlin's improved gradient noise, in three dimensions, and the flow field
 * built out of it.
 *
 ...
 * supply-chain surface in exchange for nothing. It also keeps this milestone
 * inside the one directory it was scoped to.
 *
 * The third dimension is time. Sampling `noise3(x, y, t)` rather than
 * `noise2(x, y)` is what makes the field *evolve* instead of being a fixed
 * pattern that particles slide along forever — with a static field, eight
 * hundred particles find its attractor lines within a few seconds and the drift
 * collapses into a handful of permanent streaks.
 *
 * The permutation table is seeded from a constant, so the field is identical on
 * every load and in every test run. A random seed would make "why does it look
 * like that" unanswerable.
 */
